...
if not vRP.modules.weather then
    return
end
...
function Weather:__construct()
    ...
    end -- adds all weather types to self.types 

    for k, v in pairs(self.cfg.time) do
        table.insert(self.times, {k, v})
    end -- adds all times to self.times

    table.sort(self.times, function(a, b)
        return a[2] < b[2]
    end) -- sorts self.times by value 

    table.sort(self.types, function(a, b)
        return string.upper(a) < string.upper(b)
    end) -- sorts self.types alphabetically
end
