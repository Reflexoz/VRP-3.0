...
function vRP:__construct()
    ...
    Citizen.CreateThread(function()
        while not self.db_initialized do
            self:log("DB driver \"" .. self.cfg.db.driver .. "\" not initialized yet (" .. #self.cached_prepares ..
                         " prepares cached, " .. #self.cached_queries .. " queries cached).")
            Citizen.Wait(5000)
        end
    end)
...
function vRP:authUser(source)
    ...
    for _, id in ipairs(ids) do
        local rows = self:query("vRP/userid_byidentifier", {
            identifier = id
        })
        if #rows > 0 then
    ...
    if #rows > 0 then
        ...
        for _, id in pairs(ids) do
            self:execute("vRP/add_identifier", {
                user_id = user_id,
                identifier = id
            })
        end
...
function vRP:connectUser(source)
    ...
    if prev_user then
        self:disconnectUser(prev_user.source)
    end
    --- User class deferred loading
    if not self.User then
        self.User = module("vrp", "User")
    end
    --- init user
...
function vRP:getUData(user_id, key)
    local rows = self:query("vRP/get_userdata", {
        user_id = user_id,
        key = key
    })
...
function vRP:setCData(character_id, key, value)
    self:execute("vRP/set_characterdata", {
...
function vRP:getCData(character_id, key)
    local rows = self:query("vRP/get_characterdata", {
        character_id = character_id,
        key = key
    })
    return #rows > 0 and rows[1].dvalue or ""
...
function vRP:setSData(key, value, id)
    if not id then
        id = self.cfg.server_id
    end
    self:execute("vRP/set_serverdata", {
        key = key,
        value = tohex(value),
        id = id
    })
...
function vRP:getSData(key, id)
    ...
    local rows = self:query("vRP/get_serverdata", {
        key = key,
        id = id
    })
    return #rows > 0 and rows[1].dvalue or ""
...
function vRP:setGData(key, value)
    self:execute("vRP/set_globaldata", {
        key = key,
        value = tohex(value)
    })
end
...
function vRP:getGData(key)
    local rows = self:query("vRP/get_globaldata", {
        key = key
    })
    return #rows > 0 and rows[1].dvalue or ""
end
