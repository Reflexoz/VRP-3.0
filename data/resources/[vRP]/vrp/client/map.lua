...
function PoI:frame(time)
    ...
    DrawMarker(self.marker_id, self.pos[1], self.pos[2], self.pos[3] + self.height, 0, 0, 0, 0, 0, self.rz,
        self.scale[1], self.scale[2], self.scale[3], self.color[1], self.color[2], self.color[3], self.color[4], 0)
end
    ...
    Citizen.CreateThread(function()
        while true do
            ...
            for k, v in pairs(self.areas) do
                ...
                local player_in = (GetDistanceBetweenCoords(v.x, v.y, v.z, px, py, pz, true) <= v.radius and
                                      math.abs(pz - v.z) <= v.height)
