    ...
    Citizen.CreateThread(function()
        while true do
            ...
            if not self.paused then
                -- menu controls
                if IsControlJustPressed(table.unpack(vRP.cfg.controls.phone.up)) then
                    SendNUIMessage({
                        act = "event",
                        event = "UP"
                    })
                end
                if IsControlJustPressed(table.unpack(vRP.cfg.controls.phone.down)) then
                    SendNUIMessage({
                        act = "event",
                        event = "DOWN"
                    })
                end
                if IsControlJustPressed(table.unpack(vRP.cfg.controls.phone.left)) then
                    SendNUIMessage({
                        act = "event",
                        event = "LEFT"
                    })
                end
                if IsControlJustPressed(table.unpack(vRP.cfg.controls.phone.right)) then
                    SendNUIMessage({
                        act = "event",
                        event = "RIGHT"
                    })
                end
                if IsControlJustPressed(table.unpack(vRP.cfg.controls.phone.select)) then
                    SendNUIMessage({
                        act = "event",
                        event = "SELECT"
                    })
                ...
                if IsControlJustPressed(table.unpack(vRP.cfg.controls.request.yes)) then
                    SendNUIMessage({
                        act = "event",
                        event = "F5"
                    })
                end
                if IsControlJustPressed(table.unpack(vRP.cfg.controls.request.no)) then
                    SendNUIMessage({
                        act = "event",
                        event = "F6"
                    })
