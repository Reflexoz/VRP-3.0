...
function User:getCharacters()
    ...
    local rows = vRP:query("vRP/get_characters", {
        user_id = self.id
    })
...
function User:createCharacter()
    ...
    if #characters < vRP.cfg.max_characters then
        local rows = vRP:query("vRP/create_character", {
            user_id = self.id
        })
        if #rows > 0 then
            return rows[1].id
...
function User:useCharacter(id)
    ...
    local rows = vRP:query("vRP/check_character", {
        user_id = self.id,
        id = id
    })
    if #rows > 0 then
...
function User:deleteCharacter(id)
    if self.cid ~= id then -- don't delete used character
        local affected = vRP:execute("vRP/delete_character", {
            user_id = self.id,
            id = id
        })
        if affected > 0 then
